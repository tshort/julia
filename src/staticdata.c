...
static void jl_serialize_value_(jl_serializer_state *s, jl_value_t *v)
{
    ...
    *bp = (void*)pos;
    ...
    jl_datatype_t *t = (jl_datatype_t*)jl_typeof(v);
        // jl_printf(JL_STDERR, "   serializing DataType %s, %p, %ld.\n", jl_typeof_str(v), v, item);
        // if (jl_is_module(v))
        //     jl_printf(JL_STDERR, "   -- module %s\n", jl_symbol_name((jl_sym_t*)((jl_module_t *)v)->name));
...
static void jl_write_values(jl_serializer_state *s)
{
    ...
    for (i = 0, len = backref_table_numel * 2; i < len; i += 2) {
        ...
        jl_datatype_t *t = (jl_datatype_t*)jl_typeof(v);
        // jl_printf(JL_STDERR, "   writing DataType %s.\n", jl_typeof_str(v));
...
#define JL_ARRAY_ALIGN(jl_value, nbytes) LLT_ALIGN(jl_value, nbytes)
        ...
        //    jl_printf(JL_STDERR, "   writing string %s, len %ld.\n", jl_string_data(v), jl_string_len(v));
            ios_write(s->s, (char*)v, sizeof(void*));
